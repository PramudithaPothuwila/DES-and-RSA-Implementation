...
const char* HexaDigits = "0123456789abcdef";// Hex conversion string
...
std::string DES_Block_Encryption(std::string keyInput, std::string block)
{
	char key[8];
	char plainText[8] = { 0 };
	...
	//Cipher in hexadecimal
	std::string returnCipher;
	for (int i = 0; i < 8; i++)
	{
		char first = HexaDigits[((cipherText[i] & 0xF0) >> 4)];
		char second = HexaDigits[(cipherText[i] & 0x0F)];
		returnCipher += first;
		returnCipher += second;
	}
...
std::string DES_Block_Decryption(std::string keyInput, std::string block)
{
	char key[8];
	char cipherText[8];
	...
	// plainText returned after encryption, MUST be initialized to 0s
	char plainText[8] = { 0 };

	DES_CreateKeys(key, roundKeys);
	DES_Decrypt(cipherText, roundKeys, plainText);

	std::string returnText;

	for (int i = 0; i < 8; i++)
	{
		returnText += plainText[i];
	}
...
std::string DES_Encryption(const std::string& keyInput, const std::string& input, Mode mode)
{
	std::string cipherText;
	std::string plainTextInput = input;
	switch (mode)
	{
	case ECB://Electronic Code Book mode
	{
		const int blockCount = plainTextInput.length() % 8 == 0 ? plainTextInput.length() / 8 : (plainTextInput.length() / 8) + 1;

		//Padding
		for (int k = 0; k < plainTextInput.length() % 8; k++)
		{
			plainTextInput.append(" ");
		}

		for (int i = 0; i < blockCount; i++)
		{

			std::string temp = plainTextInput.substr(BLOCK_SIZE * i, 8);
			cipherText.append(DES_Block_Encryption(keyInput, temp));
		}
		break;
	}
	case CBC://Cipher Block Chaining mode
	{
		std::cout << "CBC not Implemented";
	}
	}
	return cipherText;
}
...
// Hex decimal to Decimal Conversion
...
//Hex decimal to String Conversion
std::string hexToString(std::string string)
{
	std::string out;

	for (int i = 0; i < string.length(); i += 2) out += static_cast<char>(hexToDec(string[i]) * 16 + hexToDec(string[i + 1]));
...
std::string DES_Decryption(const std::string& keyInput, const std::string& cipherTextInput, Mode mode)
{
	...
	case ECB://Electronic Code Book mode
	{
		const int blockCount = cipherTextInput.length() % 16 == 0 ? cipherTextInput.length() / 16 : (cipherTextInput.length() / 16) + 1;
		const std::string cipherString = hexToString(cipherTextInput);
		for (int i = 0; i < blockCount; i++)
		{
			plainText.append(DES_Block_Decryption(keyInput, cipherString.substr(BLOCK_SIZE * i, 8)));
		}
		break;
	}
	case CBC://Cipher Block Chaining mode
